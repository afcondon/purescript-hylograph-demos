...
import Jtms.Explain (DerivationDag, explainFact)
...
-- | hylograph-graph's Sugiyama: jtms premise edges become the DAG,
-- | givens at layer 0, the conclusion at the far end.
dagTree :: Tree
dagTree =
  let
    dag = chosenDag
    ...
    adjacency :: Map FactId (Set FactId)
    adjacency = foldl
      (\m e -> Map.insertWith Set.union e.from (Set.singleton e.to) m)
      (Map.fromFoldable (dag.nodes <#> \f -> f.id /\ Set.empty))
      dag.edges

    layout = sugiyamaLayout
      { nodeWidth: 34.0, nodeHeight: 170.0, orientation: Horizontal, reversed: false }
      (map _.id dag.nodes)
      adjacency

    pos :: Map FactId { x :: Number, y :: Number }
    pos = Map.fromFoldable (layout <#> \n -> n.id /\ { x: n.x, y: n.y })

    posFor id = fromMaybe { x: 0.0, y: 0.0 } (Map.lookup id pos)
    ...
    nodes = dag.nodes # Array.concatMap \f ->
      let
        p = posFor f.id
        isRoot = f.id == dag.root
      in
        [ elem Rect
            [ F.x p.x, F.y p.y, F.width nodeW, F.height 22.0
            , staticStr "rx" "3"
            , F.fill (fillFor f), F.stroke (strokeFor f)
        ...
        , elem Text
            ...
            , F.fontFamily "'Helvetica Neue', Helvetica, sans-serif"
            , F.fill "#111111"
            , staticStr "textContent" (claimLabel f.claim)
            ]
            []
        ]
  in
